...
sub new($) {
	...
	my $self = {};
        ...
        my $java = new Java() ;
        ##my $java = new Java( event_port => -1);
     
        ## my $object = $java->create_object("com.comtelhi.convergepro.CTPortClone",$port); 
        my $object = $java->com_comtelhi_kelepona_Dispatcher("getDLChannel", $port); 
#        print "port = $val3 ...\n"; 
#        $val3 =  $object->getChannel->get_value(); 
#        print "channel = $val3 ...\n"; 
...
#	or croak "cannot connect to server tcp port $port";
	...
	$self->{JAVA}  = $java;		# holds the events
	$self->{CTPORTCLONE}  = $object; # holds the events
	...
	$self->{HANDLE}= $port-1200;		# our handle number
	$self->{PORT}= $port-1200;		# our handle number
...
sub on_hook() {
	...
	my $object = $self->{CTPORTCLONE};
        ...
        if (CTPORT_DEBUG) { 
          ...
          $self->logger("going on hook $port..."); 
        }
        $object->onHook(); 
        return ;
}
...
sub off_hook() {
	...
	my $object = $self->{CTPORTCLONE};
        ...
        if (CTPORT_DEBUG) { 
          my $port =  $object->getPort->get_value(); 
          $self->logger("going off hook $port..."); 
        } 
        $object->offHook(); 
        return ;
}
...
sub play($) {
  ...
  if (CTPORT_DEBUG) {
    my $port =  $object->getPort->get_value();
    $self->logger("going to play $files_str ");
  }

  foreach my $file (@files_array) {
      my $interrupted = $object->playSingleFile($file)->get_value();

      $self->logger("interrupted $interrupted");

      if ($interrupted =~ /^true/ ) {
        $self->logger("EventFlag is set");
        last; 
      } else  { 
        $self->logger("EventFlag is NOT set");
      } 
   } 
...
sub collect($$) {
   ...
   if (CTPORT_DEBUG) {
      my $port =  $object->getPort->get_value();
      $self->logger("calling collect with $maxdigits  $maxseconds ..");
   }
   my $digits = $object->collect($maxdigits,$maxseconds)->get_value(); 
   if (defined($digits)  ) { 
     $self->logger("got back--$digits..");
   } 
   return $digits;		  
}
...
sub record($$$$$) {
   ...
   ## print for debugging...
   my $no_beep_java_flag = "false:b" ; 
   if ($no_beep_flag) { 
     $no_beep_java_flag = "true:b" ; 
   } 
   if (CTPORT_DEBUG) {
      my $port =  $object->getPort->get_value();
      $self->logger("calling record with ('$file', $timeout,'$term_digits',$silence_timeout,$no_beep_java_flag");
   }
   $object->record($file, $timeout, $term_digits, $silence_timeout,$no_beep_java_flag);
}
